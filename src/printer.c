...
static force_inline const void *getData(uint16_t size) {
	const uint8_t *head = (const uint8_t *)parser_state.data_head;
	const uint8_t *tail = (const uint8_t *)parser_state.data_tail;

	assert((head + size <= tail)
		   && "Attempt to access data behind the range");

	const void *const data = parser_state.data_head;
	parser_state.data_head = (const void *)(head + size);

	return data;
}

/* --------------------------------------- Data getters ---------------------------------------- */
static force_inline char getChar(void) {
	return *(char*)getData(sizeof(char));
}

static force_inline signed short int getSignedShortInt(void) {
	return *(signed short*)getData(sizeof(signed short));
}

static force_inline unsigned short int getUnsignedShortInt(void) {
	return *(unsigned short*)getData(sizeof(unsigned short));
}

static force_inline unsigned int getUnsignedInt(void) {
	return *(unsigned int*)getData(sizeof(unsigned int));
}

static force_inline signed long int getSignedLongInt(void) {
	return *(signed long int*)getData(sizeof(signed long int));
}

static force_inline unsigned long int getUnsignedLongInt(void) {
	return *(unsigned long int*)getData(sizeof(unsigned long int));
}

static force_inline signed long long int getSignedLongLongInt(void) {
	return *(signed long long int*)getData(sizeof(signed long long int));
}

static force_inline unsigned long long int getUnsignedLongLongInt(void) {
	return *(unsigned long long int*)getData(sizeof(unsigned long long int));
}

static force_inline uint8_t getUint8(void) {
	return *(uint8_t*)getData(sizeof(uint8_t));
}

static force_inline uint16_t getUint16(void) {
	return *(uint16_t*)getData(sizeof(uint16_t));
}

static force_inline uint32_t getUint32(void) {
	return *(uint32_t*)getData(sizeof(uint32_t));
}

static force_inline uint64_t getUint64(void) {
	return *(uint64_t*)getData(sizeof(uint64_t));
}

static force_inline char* getCharPtr(void) {
	return *(char **)getData(sizeof(char*));
}

static force_inline char* getCharArray(void) {
    // Get max possible size
    const uint8_t *head = (const uint8_t *)parser_state.data_head;
	const uint8_t *tail = (const uint8_t *)parser_state.data_tail;
    const size_t size = tail - head;

    // Get actuall size of string
    const uint16_t string_size = strnlen(head, size);

    return (char *)getData(string_size);
}

/* --------------------------------------- Data printers --------------------------------------- */
static force_inline void sprintPreviousFormatChar(void) {
	*parser_state.output_buffer_head++ = *(parser_state.format_string_head - 1);
}

static force_inline void sprintPercentSign(void) {
	*parser_state.output_buffer_head++ = '%';
}

static force_inline void sprintChar(void) {
	*parser_state.output_buffer_head++ = getChar();
}

static force_inline void sprintSignedShortInt(void) {
...
static force_inline void sprintUnsignedShortInt(void) {
...
static force_inline void sprintSignedLongInt(void) { 
...
static force_inline void sprintUnsignedLongInt(void) { 
...
static force_inline void sprintSignedLongLongInt(void) { 
...
static force_inline void sprintUnsignedLongLongInt(void) { 
...
static force_inline void sprintCString(const char *const source) {
...
static void matchIntBitwidth(bool is_signed) {
    ...
    const char current_char = *parser_state.format_string_head;
    ...
    const uint16_t current_2chars = *(uint16_t*)parser_state.format_string_head;

	switch (current_2chars) {
...
// Matches: %t
static void stringHandler(void) {
    const char current_char = *parser_state.format_string_head;
    ...
    switch(current_char) {
        case 't': {
            // % st
			parser_state.format_string_head++;
            const char *const str = getCharArray();
            sprintCString(str);
        } break;
        default: {
            const char *const str = getCharPtr();
            sprintCString(str);
        } break;
    }
}
...
static force_inline void matchStart() {
	...
	if ('%' == current_char) {
		...
		*parser_state.output_buffer_head++ = current_char;
	}
}
...
void loggerPrint_printData(const char *const format_string,
	...
	parser_state.data_tail = (const uint8_t *const)data + data_size;
	parser_state.base_type = LOGGER_PARSER_BASE_DECIMAL;
